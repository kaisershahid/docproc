...
export class DocProcessor {
	...
	protected blockManager: HandlerManagerInterface<BlockHandlerType>;
	protected inlineManager: HandlerManagerInterface<InlineHandlerType>;
	...
	constructor(context?: Partial<DocProcContext> & AnyMap) {
		this.id = ++id;
		const {
			lexer,
			parser,
		...
		this.blockManager =
			blockManager ?? new HandlerManager<BlockHandlerType>();
		this.inlineManager =
			inlineManager ?? new HandlerManager<InlineHandlerType>();
	...
	protected makeCollector(): LexemeConsumer {
		...
		return (lexeme, def) => {
			...
			if (this.curHandlerDefers) {
				...
				if (
					newHandler.getName() !=
					this.parser.getCurrentHandler()?.getName()
				) {
					endCurrentHandler();
			...
			if (result == BlockActions.DEFER) {
				...
				result == BlockActions.REORDER &&
				curHandler?.modifyBlocks
			) {
